...
class ProblemSolvingAgent:
    ...
    def solve_by_searching(self, obstacles, start_pos, goal_pos, algorithm='DFS'):
        ...
        path, visited = [self.DFS, self.BFS, self.UCS, self.GBPS, self.Astar][index](
    ...
    def DFS(self, obstacles, start_pos, goal_pos):
        ...
        dic = dict()
        q = queue.Queue()
        self.addSurround(obstacles, visited, dic, q, start_pos)
        return path, visited

    def addSurround(self, obstacles, visited, tag, dic, queue, cPoint):
        for tuple in ((cPoint[0] - 1, cPoint[1]), (cPoint[0] + 1, cPoint[1]), (cPoint[0], cPoint[1] - 1), (cPoint[0], cPoint[1] + 1)):
            if (tuple not in obstacles) and (tuple not in tag):
                queue.put(tuple)
                tag.append(tuple)
    ...
    def BFS(self, obstacles, start_pos, goal_pos):
        ...
        cPoint = q.get()
        tag.append(cPoint)
        while not (cPoint == goal_pos):
            self.addSurround(obstacles, visited, tag, dic, q, cPoint)
            cPoint = q.get()
        tuple = goal_pos
        while not (tuple == start_pos):
            path.append(tuple)
            tuple = dic[tuple]
    ...
    def addSurroundAStar(self, obstacles, goal_pos, visited, tag, dic, sortedList, cPoint, step):
        def cmp(A, B, goal=goal_pos, step=step):
            pass
            disA = self.euclidean_distance(A, goal_pos)
            disB = self.euclidean_distance(B, goal_pos)
            stepA = step[A]
            stepB = step[B]
            return (self.euclidean_distance(A, goal_pos) + step[A]) - (self.euclidean_distance(B, goal_pos) + step[B])
        for subPoint in ((cPoint[0] - 1, cPoint[1]), (cPoint[0] + 1, cPoint[1]), (cPoint[0], cPoint[1] - 1), (cPoint[0], cPoint[1] + 1)):
            if (subPoint not in obstacles) and (subPoint not in tag):
                sortedList.append(subPoint)
                sortedList.sort(key=cmp_to_key(cmp))
                step[subPoint] = step[cPoint] + 1
                tag.append(subPoint)
                visited.append(subPoint)
                dic[subPoint] = cPoint
    ...
    def Astar(self, obstacles, start_pos, goal_pos):
        ...
        tag = []
        sortedList = []
        sortedList.append(start_pos)
        step[start_pos] = 0
        cPoint = sortedList[0]
        sortedList.remove(sortedList[0])
        tag.append(cPoint)
        while not (cPoint == goal_pos):
            self.addSurroundAStar(obstacles, goal_pos, visited,
                                  tag, dic, sortedList, cPoint, step)
            cPoint = sortedList[0]
            sortedList.remove(sortedList[0])
        tuple = goal_pos
        while not (tuple == start_pos):
